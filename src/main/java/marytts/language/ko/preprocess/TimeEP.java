...
import marytts.language.ko.lib.KoreanNumbersToWords;
...
public class TimeEP extends ExpansionPattern
{
    ...
    protected int match(String s, int type)
    {
        switch (type) {
        case 0:
            if (matchTimeHMS(s)) return 1;
            if (matchTimeHM(s)) return 2;
    ...
    protected int canDealWith(String s, int type){
    	...
    	// "hmsUnknown" or "hms12".)
    	/*case 0:
    		if (canDealWithTimeHMS12(s)) return 4;
    		if (canDealWithTimeHMS24(s)) return 5;
    		break;
        case 1:
        	if (canDealWithTimeHMS12(s)) return 4;
    		if (canDealWithTimeHMS24(s)) return 5;
