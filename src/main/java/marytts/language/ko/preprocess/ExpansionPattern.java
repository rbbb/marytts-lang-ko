...
public abstract class ExpansionPattern {
    ...
    //no abbreviations in Korean, though sometimes only the first
    //character of each word is selected
...
//		abbrev = new AbbrevEP();
        ...
        //order of addition matters, first in list get matched first
    ...
    public boolean process(Element t, final List<Element> expanded) {
        ...
        if (allowMultipleTokens()) {
            ...
            //Korean counted number patterns change the algorithm slightly,
            //the counter word might be at the beginning of a token followed
            //by the rest of a hangul block, we stop adding candidate tokens after the
            //first non-digit token we find
...
//                System.out.println(sb.toString()+" "+matchedType);
                if (matchedType != -1)
